...
export default function PatientLogin() {
  ...
  return (
    <PageTransition>
    <div className={`min-h-screen relative overflow-hidden bg-slate-50 flex flex-col font-sans text-gray-800 ${showMultiple ? 'pt-[60px]' : ''}`}>
          ...
          {showMultiple ? (
            ...
            >
              <div className="flex flex-col items-center text-center mb-7 mt-2">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#004e8c] to-[#0070c9] flex items-center justify-center shadow-lg shadow-blue-900/20 mb-3">
                  <Users size={26} className="text-white" strokeWidth={2} />
                </div>
                <h2 className="text-xl font-extrabold text-slate-800 tracking-tight mb-1.5">
                  Pilih Data Pasien
                </h2>
                <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#004e8c] bg-[#004e8c]/8 px-3 py-1 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#004e8c] animate-pulse"></span>
                  ...
                  return (
                    <div 
                      ...
                      className={`w-full rounded-[20px] p-5 cursor-pointer transition-all duration-200 border-l-[4px] ${
                        isSelected 
                          ? 'bg-[#eef4fa] border-l-[#004e8c] border border-[#004e8c]/20 shadow-md' 
                          : 'bg-white border-l-transparent border border-slate-100 shadow-sm hover:bg-slate-50/60 hover:shadow-md'
                      }`}
                    >
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 mb-1">
                            <User size={12} strokeWidth={2} className={isSelected ? 'text-[#004e8c]' : 'text-slate-400'} />
                            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">No. RM</span>
                          </div>
                          <p className="text-xs font-bold text-slate-800 pl-[18px]">{p.rmNumber}</p>
                        </div>
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 mb-1">
                            <User size={12} strokeWidth={2} className={isSelected ? 'text-[#004e8c]' : 'text-slate-400'} />
                            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Nama</span>
                          </div>
                          <p className="text-xs font-bold text-slate-800 pl-[18px]">{p.name}</p>
                        </div>

                        {/* Tanggal Lahir */}
                        <div>
                          <div className="flex items-center gap-1.5 mb-1">
                            <Calendar size={12} strokeWidth={2} className={isSelected ? 'text-[#004e8c]' : 'text-slate-400'} />
                            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Tgl Lahir</span>
                          </div>
                          <p className="text-xs font-bold text-slate-800 pl-[18px]">{formatDate(p.dob)}</p>
                        </div>

                        {/* Telepon */}
                        <div>
                          <div className="flex items-center gap-1.5 mb-1">
                            <Phone size={12} strokeWidth={2} className={isSelected ? 'text-[#004e8c]' : 'text-slate-400'} />
                            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Telepon</span>
                          </div>
                          <p className="text-xs font-bold text-slate-800 pl-[18px]">{maskPhone(p.phone)}</p>
                        </div>

                        {/* Alamat - full width */}
                        <div className="col-span-2">
                          <div className="flex items-center gap-1.5 mb-1">
                            <MapPin size={12} strokeWidth={2} className={isSelected ? 'text-[#004e8c]' : 'text-slate-400'} />
                            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Alamat</span>
                          </div>
                          <p className="text-xs font-bold text-slate-800 pl-[18px] truncate" title={maskAddress(p.address)}>{maskAddress(p.address)}</p>
                        </div>

                        {/* Ruangan - full width */}
                        <div className="col-span-2">
                          <div className="flex items-center gap-1.5 mb-1">
                            <Building size={12} strokeWidth={2} className={isSelected ? 'text-[#004e8c]' : 'text-slate-400'} />
                            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Ruangan</span>
                          </div>
                          <p className="text-xs font-bold text-slate-800 pl-[18px]">{p.roomName} - {formatRoomClass(p.roomClass)}</p>
                        </div>
                      </div>

                      {/* Allergy Warning */}
                      {hasAllergies && (
                        <div className={`mt-3 rounded-lg p-2.5 flex items-center gap-2 border ${isSelected ? 'bg-red-50 border-red-200' : 'bg-red-50/50 border-red-100'}`}>
                          <AlertTriangle size={13} className="text-red-500 flex-shrink-0" strokeWidth={2.5} />
                          <span className="text-[10px] font-bold text-red-600">Alergi: {p.allergies}</span>
                        </div>
                      )}

                      {/* Info Note */}
                      <div className={`mt-2.5 rounded-lg p-2.5 flex items-center gap-2 ${isSelected ? 'bg-blue-50/80' : 'bg-slate-50/80'}`}>
                        <Info size={13} className={`flex-shrink-0 ${isSelected ? 'text-[#004e8c]' : 'text-blue-400'}`} strokeWidth={2.5} />
                        <p className={`text-[10px] font-medium ${isSelected ? 'text-[#004e8c]/80' : 'text-blue-600/70'}`}>
              ...
              <button
                ...
                className="w-full bg-[#004e8c] hover:bg-[#003d6f] text-white py-3.5 rounded-xl font-bold text-sm shadow-lg shadow-blue-900/20 active:scale-[0.98] transition-all border-none outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Pilih Pasien Ini
              </button>
            </motion.div>
            ...
            >
              ...
              <form onSubmit={handleSearch} className="space-y-3">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full mt-1 bg-[#004e8c] hover:bg-[#003d6f] text-white py-2.5 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all active:scale-[0.98] shadow-lg shadow-blue-900/20 text-xs border-none outline-none disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
                    <>
